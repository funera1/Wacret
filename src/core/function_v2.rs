...
pub struct CodePos<'a> {
    ...
    pub offset: u32,
    pub type_stack: Vec<ValType>,
    pub callee_return_size: u32,
}

impl<'a> BytecodeFunction<'a> {
    pub fn new(module: &'a Module<'a>, func_body: &'a FunctionBody<'a>, locals: Vec<ValType>, else_blockty: BlockType, codes: Vec<CodePos<'a>>) -> Self {
    ...
    // 命令を1つ進める
    pub fn dispatch(&mut self, v: &mut Vec<ValType>, op: &Operator<'_>) -> Result<u32, String> {
        match op {
