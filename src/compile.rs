...
impl<'a> FastBytecodeFunction<'a> {
    ...
    fn emit_label(codepos: &CodePos<'a>, input: Vec<u8> , output: Vec<u8>) -> FastCodePos<'a> {
        let f = FastCodePos {
        ...
        for codepos in &func.codes {
            match codepos.opcode {
                ...
                Operator::LocalGet{ local_index } => {
                    ...
                    stack.push(func.locals[local_index as usize]);
                }
                Operator::LocalSet{ .. } => {
                ...
                Operator::GlobalGet{ global_index } => {
                    let operand: Vec<u8>;
                    match module.globals[global_index as usize].content_type {
                        ValType::I32 | ValType::F32 => operand = vec![BYTE_U32],
                        ValType::I64 | ValType::F64 => operand = vec![BYTE_U64],
                        // TODO: refはBYTE_U128じゃなさそう
                        ValType::V128 | ValType::Ref(_)=> operand = vec![BYTE_U128],
                    }
                    fast_bytecode.push(Self::emit_label(codepos, vec![], operand));
                }
                Operator::GlobalSet{ .. } => {
                    fast_bytecode.push(Self::emit_label(codepos, vec![0], vec![]));
                }
                Operator::TableGet{ .. } => {
                    fast_bytecode.push(Self::emit_label(codepos, vec![0], vec![]));
                }
                Operator::TableSet{ .. } => {
                    fast_bytecode.push(Self::emit_label(codepos, vec![BYTE_U32]));
                }
