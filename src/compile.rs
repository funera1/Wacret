...
impl<'a> FastBytecodeFunction<'a> {
    pub fn construct(funcs: &Vec<Function<>>) -> Result<Vec<FastBytecodeFunction<'a>>> { 
        ...
        for func in funcs {
            match func {
                ...
                Function::BytecodeFunction(b)=> {
                    let fast_bytecode = Self::compile_fast_bytecode(b);
    ...
    fn emit_label(codepos: &CodePos<'a>, operand: Vec<u8>) -> FastCodePos<'a> {
        let f = FastCodePos {
    ...
    pub fn compile_fast_bytecode(func: &BytecodeFunction) {
        // 仮スタック
        ...
        for codepos in &func.codes {
            match codepos.opcode {
                ...
                Operator::GlobalGet{ global_index } => {
                    fast_bytecode.push(Self::emit_label(codepos, operand));
                }
                Operator::GlobalSet{ .. } => {
                }
                Operator::TableGet{ .. } => {
