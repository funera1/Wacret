...
def parse(file: str):
    with open(file, 'rb') as f:
        ...
        print(f"After entry_fidx, position: {f.tell()}")
        # リターンアドレス
        return_fidx = to_int(f.read(4))
        return_offset = to_int(f.read(4))
        print(f"After return address, position: {f.tell()}")
        # 型スタック
        type_stack_size = to_int(f.read(4))
        print(f"After type_stack_size, position: {f.tell()}")
        type_stack = []
        for i in range(type_stack_size):
            type = to_int(f.read(1))
            type_stack.append(type)
        print(f"After type stack, position: {f.tell()}")
        ...
        print(f"After value stack, position: {f.tell()}")

        # TODO: ラベルスタック
        label_stack_size = to_int(f.read(4))
        print(f"After label_stack_size ({label_stack_size}), position: {f.tell()}")
        label_stack = []
        for i in range(label_stack_size):
            print(f"Reading label {i} at position {f.tell()}")
            ...
            print(f"After reading label {i}, position: {f.tell()}")
